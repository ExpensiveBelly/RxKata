...
public class MapFlatMapExercises {
    ...
    public Observable<Integer> exerciseFlatten() {
        ...
        return pairObservable.flatMap(it -> fromArray(it.getValue0(), it.getValue1(), it.getValue2()));
    }
    ...
    public Observable<String> exerciseEmail() {
        ...
        return just(booking)
                .filter(it -> it.getUser().isPresent() && it.getUser().get().getEmail().isPresent())
                .map(it -> it.getUser().get().getEmail().get());
    }
...
Observable<String> words = Observable.fromArray(tokens);
		...
		.scan((total, current) -> total + current);return words
